...
const NotificationsList = () => {
  ...
  const { renderMessage, populateNotificationDetails, setUnreadCount } =
    useNotifications();
  ...
  return (
    <div className="flex flex-col space-y-4 secondary min-h-screen text-primary">
      <div className="flex items-center justify-center">
        <button
          onClick={handleRefresh}
          className="flex items-center p-2 border border-gray-600 rounded-lg bg-gray-700 info hover:bg-gray-600 transition"
        >
      ...
      {notifications.length > 0
        ? notifications.map((notification) => {
            ...
            return (
              <div
                key={notification.id}
                className="flex items-center p-4 primary rounded-lg transition-shadow duration-200 cursor-pointer"
